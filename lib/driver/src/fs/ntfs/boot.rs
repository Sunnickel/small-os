use crate::fs::ntfs::error::NtfsError;
...
impl BootSector {
    ...
    /// B).
    pub(crate) fn mft_record_size(&self) -> usize {
        if self.clusters_per_mft_record > 0 {
            self.clusters_per_mft_record as usize * self.bytes_per_cluster() as usize
        } else {
            let exp = -(self.clusters_per_mft_record as i32) as u32;
            assert!(exp < 64, "Invalid clusters_per_mft_record: {}", self.clusters_per_mft_record);
            1usize << exp
        }
    }
    ...
    pub(crate) fn mft_mirror_byte_offset(&self) -> u64 {
    ...
    pub(crate) fn cluster_to_disk_offset(&self, lcn: u64) -> u64 {
