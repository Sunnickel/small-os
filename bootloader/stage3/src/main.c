...
static void launch_elf(const char* name83, const char* label, BootInfo* info,
                       uint64_t fat32_lba, uint64_t boot_disk, void (*read_sector)(void*))
{
  log("loading binary");
...
static void boot_from_disk(BootInfo* info)
{
  ...
  if (probe.result == PROBE_FOUND)
  {
    ...
    launch_elf(KERNEL_NAME83, "kernel", info, FAT32_DISK_LBA, BOOT_DISK_VIRTIO,
...
static void boot_from_usb(BootInfo* info)
{
  ...
  launch_elf(INSTALLER_NAME83, "installer", info, FAT32_BOOT_LBA,
             BOOT_DISK_VIRTIO, virtio_blk_read_sector);
}

static uintptr_t virtio_mmio_find_block(void)
{
  for (uintptr_t addr = 0x10001000; addr < 0x10010000; addr += 0x1000)
  {
    volatile uint32_t* mmio = (volatile uint32_t*)addr;

    if (mmio[0x000 / 4] != 0x74726976) // "virt"
      continue;

    if (mmio[0x008 / 4] != 2) // block device
      continue;
...
void stage3_main(BootInfo* boot_info)
{
  ...
  if (!boot_info) panic("boot_info is null");

  if (boot_info->memory_map_len == 0) log("warning: no mmap");
  else log("mmap entries ok");

  if (boot_info->fb_addr == 0) log("warning: no fb");
  else log("fb ok");
  ...
  // Test 5: virtio
  uintptr_t mmio = virtio_mmio_find_block();
  if (!mmio)
    panic("no virtio mmio device found");

  log("virtio mmio found");

  if (!virtio_blk_init(mmio))
    panic("virtio init failed");

  // Continue normal boot
  log("entering boot logic");
