...
use x86_64::{
    structures::paging::{FrameAllocator, Mapper, Page, PhysFrame, Size4KiB}, PhysAddr,
    VirtAddr,
};
    ...
    fn usable_frames(&self) -> impl Iterator<Item = PhysFrame> {
        self.memory_map
            ...
            .flat_map(|r| {
                let start = r.start;
                let end = r.end;
                (start..end).step_by(4096).map(|addr| PhysFrame::containing_address(PhysAddr::new(addr)))
            })
    }
}