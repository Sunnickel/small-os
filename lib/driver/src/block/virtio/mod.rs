...
use bus::pci::{
	PciBusDevice,
	caps::{CapabilityId, find_capability},
};
...
impl VirtioBlkState {
	...
	unsafe fn init_device(
		...
		unsafe {
			...
			let dma_virt = dma_buf.virt as usize;
	...
	pub fn read_sectors(&mut self, lba: u64, buf: &mut [u8]) -> Result<(), BlockError> {
		...
		unsafe {
			...
			core::ptr::write_volatile(req_ptr, req);
	...
	pub fn write_sectors(&mut self, lba: u64, buf: &[u8]) -> Result<(), BlockError> {
		...
		unsafe {
			...
			self.setup_chain(
				...
				core::ptr::null_mut(), // No output buffer for write
				sectors * 512,
	...
	unsafe fn setup_chain(&mut self, req_phys: u64, data_phys: u64, user_buf: *mut u8, len: usize, is_read: bool) {
	...
	unsafe fn notify_and_wait(&self) {
		// Write notify offset
