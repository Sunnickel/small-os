...
use crate::{Binding, Driver, DriverError, MatchRule};
...
impl DriverRegistry {
	...
	pub fn try_bind(
		...
		self.bindings.write().insert(
			device_id,
			Binding::new(device_id, device, name, state),
		);
	...
	pub fn take_block_device(&self) -> Option<(DeviceId, crate::block::BlockDeviceEnum)> {
		...
		for id in device_ids {
			if let Some(binding) = bindings.remove(&id) {
				...
				// For now, we drop non-block drivers (they're removed from registry)
			}
		}
		None
	}
	...
	fn find_driver(
		...
		if let Some(pci_dev) = pci {
			...
			for driver in drivers.iter() {
				let matches = driver.rules().iter().any(|rule| {
					rule.matches_pci(vendor, dev_id, class, subclass)
				});
