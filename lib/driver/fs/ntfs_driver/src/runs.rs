    ...
    for run in data_runs {
        match run {
            ...
            DataRun::NonResident(runs) => {
                for (lcn, length_in_clusters) in runs {
                    let byte_offset = boot.cluster_to_disk_offset(*lcn);
                    let bytes_to_read = *length_in_clusters * boot.bytes_per_cluster();

                    let mut cluster_data = vec![0u8; bytes_to_read as usize];
                    device
                        .read_at(byte_offset, &mut cluster_data)
