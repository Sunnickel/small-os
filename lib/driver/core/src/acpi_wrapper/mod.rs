...
use acpi::{platform::PciConfigRegions, AcpiTables, Handler, PhysicalMapping};
...
impl Handler for KernelAcpiHandler {
    ...
    fn read_pci_u8(&self, address: acpi::PciAddress, offset: u16) -> u8 {
        unsafe {
            crate::pci::ecam_read8(address.bus(), address.device(), address.function(), offset)
        }
    }
    fn read_pci_u16(&self, address: acpi::PciAddress, offset: u16) -> u16 {
        unsafe {
            crate::pci::ecam_read16(address.bus(), address.device(), address.function(), offset)
        }
    }
    fn read_pci_u32(&self, address: acpi::PciAddress, offset: u16) -> u32 {
        unsafe {
            crate::pci::ecam_read32(address.bus(), address.device(), address.function(), offset)
        }
    }
...
pub fn init_from_rsdp(rsdp_phys: usize, phys_offset: u64) {
    ...
    crate::debug(&alloc::format!("ECAM base = {:#x}, phys_offset = {:#x}", ecam_base, phys_offset));
    crate::debug(&alloc::format!("ACPI MCFG: segment 0 ECAM base = {:#x}", ecam_base,));
