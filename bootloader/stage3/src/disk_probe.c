...
static void read(ReadSectorFn fn, uint64_t lba) { fn(lba, g_sector); }
...
static DiskProbe probe_mbr(ReadSectorFn fn) {
	...
	for (int i = 0; i < MBR_MAX_PARTS; i++)
	{
      ...
      if (!is_ntfs_boot(fn, lba))
	      continue; // signature mismatch

      result.ntfs_lba = lba;
      result.result = ntfs_has_kernel(fn, lba) ? PROBE_FOUND : PROBE_NO_KERNEL;
      return result;
		}
	}
