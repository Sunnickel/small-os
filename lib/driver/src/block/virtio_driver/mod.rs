...
impl BlockDevice for VirtioDriver {
    fn read_at(&mut self, offset: u64, buf: &mut [u8]) -> Result<(), hal::io::IoError> {
        if offset % SECTOR_SIZE as u64 != 0 {
            return Err(BlockError::InvalidRequest.into());
        }
        let mut sector = offset / SECTOR_SIZE as u64;
        let mut remaining = buf;
        while !remaining.is_empty() {
            let (chunk, rest) = remaining.split_at_mut(SECTOR_SIZE.min(remaining.len()));
            // If chunk is less than a sector, use a temp buffer
            if chunk.len() < SECTOR_SIZE {
                let mut tmp = [0u8; 512];
                unsafe { self.do_request(sector, &mut tmp, false)? };
                chunk.copy_from_slice(&tmp[..chunk.len()]);
            } else {
                unsafe { self.do_request(sector, chunk, false)? };
            }
            sector += 1;
            remaining = rest;
        }
    ...
    fn write_at(&mut self, offset: u64, buf: &[u8]) -> Result<(), hal::io::IoError> {
        ...
        let mut pos = 0;
        while pos < buf.len() {
            let end = (pos + SECTOR_SIZE).min(buf.len());
            if end - pos < SECTOR_SIZE {
                // Partial sector: read-modify-write
                let mut tmp = [0u8; 512];
                unsafe { self.do_request(sector, &mut tmp, false)? };
                tmp[..end - pos].copy_from_slice(&buf[pos..end]);
                unsafe { self.do_request(sector, &mut tmp, true)? };
            } else {
                let mut tmp = buf[pos..end].to_vec();
                unsafe { self.do_request(sector, &mut tmp, true)? };
            }
            sector += 1;
            pos = end;
