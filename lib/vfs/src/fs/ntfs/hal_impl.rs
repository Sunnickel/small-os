...
use alloc::sync::Arc;
use spin::Mutex;
use hal::fs::{FileSystem, File, FsError};
use hal::io::{IoError, Read, Seek, SeekFrom};
...
impl NtfsFsImpl {
	pub fn new(block_dev: BlockDeviceEnum) -> Result<Self, FsError> {
		let driver = NtfsDriver::mount(block_dev, 0)
			.map_err(|_| FsError::Io(IoError::Other))?;
		Ok(Self {
...
impl Read for NtfsFileImpl {
	fn read(&mut self, buf: &mut [u8]) -> Result<usize, IoError> {
		...
		let data = driver.read_file_all(&self.file)
