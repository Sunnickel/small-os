...
impl<D: BlockDevice> NtfsDriver<D> {
    ...
    pub fn open(&mut self, path: &str) -> Result<NtfsFile, NtfsError> {
        ...
        debug(&format!("open: path={}", normalized));
        ...
        for component in normalized.split('/').filter(|s| !s.is_empty()) {
            ...
            debug(&format!(
                "open: traversing component='{}' from record={}",
                component, current.record_number
            ));
            ...
            debug(&format!("open: found '{}' at record={}", component, child));
